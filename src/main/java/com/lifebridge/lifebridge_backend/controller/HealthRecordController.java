...
public class HealthRecordController {
    ...
    // Endpoint 1: Save or Update a health record
    @PostMapping("/save/{userId}")
    public ResponseEntity<HealthRecord> saveRecord(@PathVariable Long userId, @RequestBody HealthRecord record) {
        HealthRecord savedRecord = healthRecordService.saveOrUpdateRecord(userId, record);
        return new ResponseEntity<>(savedRecord, HttpStatus.CREATED);
    }

    // Endpoint 2: Get a single record for a specific user
